...
\textbf{TestsuitesManagement}. Because of the \textbf{JsonPreprocessor} is a pure Python application, whereas the \textbf{RobotFramework AIO}
is a framework with own synatx rules and conditions, in some cases the JSONP format deviates in both worlds. Where required you will find corresponding
...
All lines starting with \pcode{//}, are ignored by the \textbf{JsonPreprocessor}. The output of this example is the same than in the previous example.
...
To shorten the code, the \textbf{JsonPreprocessor} supports an implicite creation of dictionaries.
